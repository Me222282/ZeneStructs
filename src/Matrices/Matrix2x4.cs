...
namespace Zene.Structs
{
    public unsafe class Matrix2x4
    {
        ...
        public void Set(Matrix2x4 matrix) => matrix._matrix.CopyTo(_matrix, 0);
...
[{_matrix[4].ToString(format)}, {_matrix[5].ToString(format)}, {_matrix[6].ToString(format)}, {_matrix[7].ToString(format)}]";
        ...
        public static Matrix2x4 Identity { get; } = CreateIdentity();

        public static Matrix2x4 CreateIdentity()
        {
            return new Matrix2x4(new Vector4(1, 0, 0, 0), new Vector4(0, 1, 0, 0));
        }
